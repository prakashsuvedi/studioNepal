...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  return (
    <nav 
      ...
      className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-slate-950/95 backdrop-blur-xl border-t border-slate-800 px-2 py-1.5 shadow-[0_-4px_25px_rgba(0,0,0,0.5)] select-none safe-area-bottom text-slate-100"
    >
      <div className="flex items-center justify-around max-w-lg mx-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <span className="absolute inset-x-2 -top-1 h-0.5 bg-gradient-to-r from-cyan-400 to-indigo-500 rounded-full shadow-[0_0_8px_rgba(6,182,212,0.8)]" />
              )}
              ...
              <div className="relative">
                ...
                {item.requiresAuth && !user && (
                  <span className="absolute -bottom-1 -right-1 w-3 h-3 bg-slate-900 rounded-full flex items-center justify-center text-slate-400 border border-slate-700">
                    <Lock className="w-2 h-2" />
                  </span>
                )}
              </div>

              <span className={`text-[10px] mt-0.5 font-medium tracking-tight leading-none ${isActive ? 'font-black text-cyan-300' : ''}`}>
                {item.label}
              </span>
            </button>
          );
        ...
        {user ? (
          ...
          >
            <div className="flex items-center justify-center w-7 h-7 rounded-full bg-amber-950/40 border border-amber-600/60 text-amber-300 relative">
              <Zap className="w-3.5 h-3.5 fill-amber-400" />
              <span className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-cyan-400 border border-slate-900" />
            </div>
            <span className="text-[10px] mt-0.5 font-bold text-amber-300 flex items-center gap-0.5">
          ...
          <button
            ...
            className="flex flex-col items-center justify-center min-w-[52px] min-h-[48px] py-1 px-1 rounded-2xl text-slate-400 hover:text-cyan-300 cursor-pointer"
            title="Sign In"
          >
            <div className="flex items-center justify-center w-7 h-7 rounded-full bg-gradient-to-r from-cyan-500 to-indigo-600 text-white shadow-[0_0_10px_rgba(6,182,212,0.4)]">
              <User className="w-3.5 h-3.5" />
            </div>
            <span className="text-[10px] mt-0.5 font-bold text-cyan-300">
              Sign In
            </span>
          </button>
        )}
